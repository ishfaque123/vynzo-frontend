...
export default function ReelsPage() {
  ...
  useEffect(() => { let cancelled = false; (async () => { const [feed, status, config] = await Promise.all([fetchReelFeed(), fetchMyReelStatus(), fetchReelsConfig()]); if (cancelled) return; if (feed.success) { const feedReels = feed.data.reels || []; const authorIds = [...new Set(feedReels.filter((item: Reel) => !item.isMine).map((item: Reel) => item.author.id))] as string[]; const statusResults: Array<[string, string] | null> = await Promise.all(authorIds.map(async (userId) => { const result = await fetchFollowStatus(userId); const followStatus = result.success ? result.data?.status : null; return typeof followStatus === 'string' ? [userId, followStatus] : null; })); if (cancelled) return; const statusMap = new Map<string, string>(); statusResults.forEach((item) => { if (item && typeof item[0] === 'string' && typeof item[1] === 'string') statusMap.set(item[0], item[1]); }); setReels(feedReels.map((item: Reel) => statusMap.has(item.author.id) ? { ...item, friendStatus: statusMap.get(item.author.id) } : item)); setHasMore(!!feed.data.hasMore); } const enabled = !!(config.success && config.data?.enabled); const remaining = status.success ? Number(status.data?.remaining) : 0; setReelsEnabled(enabled); setUploadMaxDuration(Number(config.data?.maxDurationSec) > 0 ? Number(config.data.maxDurationSec) : 60); setUploadReady(enabled && status.success && Number.isFinite(remaining) && remaining > 0); setLoading(false); })(); return () => { cancelled = true; }; }, []);
