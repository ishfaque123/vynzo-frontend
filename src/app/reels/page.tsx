...
function ReelItem({ reel, active, forcePause, onLikeChange, onFavoriteChange, onDeleted, onFollowed, onCommentCountChange, onNext }: { reel: Reel; active: boolean; forcePause: boolean; onLikeChange: (id: string, liked: boolean, count: number) => void; onFavoriteChange: (id: string, favorited: boolean) => void; onDeleted: (id: string) => void; onFollowed: (userId: string) => void; onCommentCountChange: (id: string, delta: number) => void; onNext: () => void }) {
  ...
  const heartTimersRef = useRef<ReturnType<typeof window.setTimeout>[]>([]);
  ...
  return <div className="relative flex h-full w-full flex-shrink-0 items-center justify-center bg-black">
    ...
    <div className="absolute bottom-24 right-3 z-10 flex flex-col items-center gap-5"><button onClick={(e) => { e.stopPropagation(); void handleLike(); }} className="flex flex-col items-center gap-1"><HeartIcon filled={reel.liked} /><span className="text-xs font-medium text-white">{reel.likeCount}</span></button><button onClick={(e) => { e.stopPropagation(); void openComments(); }} className="flex flex-col items-center gap-1"><CommentIcon /><span className="text-xs font-medium text-white">{reel.commentCount ?? ''}</span></button><button onClick={(e) => { e.stopPropagation(); void handleFavorite(); }} className="flex flex-col items-center gap-1"><BookmarkIcon filled={reel.favorited} /></button><button onClick={(e) => { e.stopPropagation(); setShareOpen(true); }} className="flex flex-col items-center gap-1"><ShareIcon /></button>{reel.isMine && <button onClick={(e) => { e.stopPropagation(); void handleDelete(); }} className="flex flex-col items-center gap-1"><TrashIcon /></button>}</div>
    {shareOpen && <ShareModal reelId={reel.id} onClose={() => setShareOpen(false)} />}
    {commentsOpen && <ReelCommentsModal reelId={reel.id} reelOwner={reel.isMine} currentUserId={currentUser?.id} comments={comments} onClose={() => setCommentsOpen(false)} onChanged={(delta) => onCommentCountChange(reel.id, delta)} />}
  </div>;
}
...
export default function ReelsPage() {
  ...
  useAuth();
  useEffect(() => { let cancelled = false; (async () => { const config = await fetchReelsConfig(); if (!config.success || !config.data.enabled) { setLoading(false); return; } const [feed, status] = await Promise.all([fetchReelFeed(), fetchMyReelStatus()]); if (cancelled) return; if (feed.success) setReels(feed.data.reels || []); setUploadReady(status.success && status.data.canUpload); setLoading(false); })(); return () => { cancelled = true; }; }, []);
  useEffect(() => { const root = containerRef.current; if (!root) return; const items = Array.from(root.querySelectorAll<HTMLElement>('[data-reel-index]')); const observer = new IntersectionObserver((entries) => { const visible = entries.filter((entry) => entry.isIntersecting).sort((a, b) => b.intersectionRatio - a.intersectionRatio)[0]; if (visible) setActiveIndex(Number((visible.target as HTMLElement).dataset.reelIndex)); }, { root, threshold: [0.6, 0.8, 1] }); items.forEach((item) => observer.observe(item)); return () => observer.disconnect(); }, [reels.length]);
  function scrollToIndex(index: number) { if (index < 0 || index >= reels.length) return; containerRef.current?.children[index]?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }
  function nextReel() { scrollToIndex(Math.min(activeIndex + 1, reels.length - 1)); }
  function handleUpload() { setPendingReelVideo(''); router.push('/reels/upload'); }
  if (loading) return <div className="flex h-[100dvh] items-center justify-center bg-black text-white">Loading...</div>;
  if (!reels.length) return <div className="flex h-[100dvh] items-center justify-center bg-black text-white">No reels yet.</div>;
  return <main className="relative h-[100dvh] overflow-hidden bg-black"><div ref={containerRef} className="h-full snap-y snap-mandatory overflow-y-auto overscroll-contain scrollbar-none">{reels.map((reel, index) => <div key={reel.id} data-reel-index={index} className="h-full snap-start"><ReelItem reel={reel} active={activeIndex === index} forcePause={forcePause} onLikeChange={(id, liked, count) => setReels((items) => items.map((item) => item.id === id ? { ...item, liked, likeCount: count } : item))} onFavoriteChange={(id, favorited) => setReels((items) => items.map((item) => item.id === id ? { ...item, favorited } : item))} onDeleted={(id) => setReels((items) => items.filter((item) => item.id !== id))} onFollowed={(userId) => setReels((items) => items.map((item) => item.author.id === userId ? { ...item, friendStatus: 'following' } : item))} onCommentCountChange={(id, delta) => setReels((items) => items.map((item) => item.id === id ? { ...item, commentCount: Math.max(0, (item.commentCount ?? 0) + delta) } : item))} onNext={nextReel} /></div>)}</div><button onClick={handleUpload} disabled={!uploadReady} aria-label="Post a reel" className="absolute right-4 top-4 z-50 flex h-11 w-11 items-center justify-center rounded-full bg-black/60 text-white disabled:opacity-40"><PlusIcon /></button></main>;
}
