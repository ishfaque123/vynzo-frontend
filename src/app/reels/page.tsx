...
function SeekIcon({ forward }: { forward: boolean }) { return <div className="relative flex h-14 w-14 items-center justify-center"><svg width="50" height="50" viewBox="0 0 42 42" fill="none" stroke="white" strokeWidth="2.2"><path d={forward ? 'M18 11a11 11 0 11-7.5 19.1' : 'M24 11a11 11 0 11-7.5 19.1'} /><path d={forward ? 'M24 7l-1 7 7-1' : 'M18 7l1 7-7-1'} /><path d={forward ? 'M17 21h5' : 'M25 21h-5'} /></svg><span className="absolute text-[10px] font-bold text-white">10</span></div>; }
...
function ReelItem({ reel, active, forcePause, onLikeChange, onFavoriteChange, onDeleted, onFollowed, onCommentCountChange, onNext }: { reel: Reel; active: boolean; forcePause: boolean; onLikeChange: (id: string, liked: boolean, count: number) => void; onFavoriteChange: (id: string, favorited: boolean) => void; onDeleted: (id: string) => void; onFollowed: (userId: string) => void; onCommentCountChange: (id: string, delta: number) => void; onNext: () => void }) {
  ...
  async function handleFollow(e: React.MouseEvent) { e.stopPropagation(); if (following) return; setFollowing(true); const result = await toggleFollow(reel.author.id); setFollowing(false); if (result.success && result.data.following) onFollowed(reel.author.id); }
  useEffect(() => { const video = videoRef.current; if (!video || !active) return; video.currentTime = 0; video.muted = false; setIsMuted(false); setShowControls(false); video.play().catch(() => { video.muted = true; setIsMuted(true); video.play().catch(() => {}); }); }, [active]);
  useEffect(() => { const video = videoRef.current; if (!video) return; if (!active || forcePause) video.pause(); else video.play().catch(() => {}); }, [active, forcePause]);
  function toggleMute(e: React.MouseEvent) { e.stopPropagation(); const video = videoRef.current; if (!video) return; video.muted = !video.muted; setIsMuted(video.muted); }
  function togglePlayback(e?: React.MouseEvent) { e?.stopPropagation(); const video = videoRef.current; if (!video) return; if (video.paused) video.play().catch(() => {}); else video.pause(); revealControls(); }
  function seek(seconds: number, e?: React.MouseEvent) { e?.stopPropagation(); const video = videoRef.current; if (!video) return; video.currentTime = Math.max(0, Math.min(video.duration || Infinity, video.currentTime + seconds)); setSeekFeedback(seconds > 0 ? 'forward' : 'back'); window.setTimeout(() => setSeekFeedback(null), 550); revealControls(); }
  function handleVideoTap(e: React.MouseEvent<HTMLVideoElement>) {
    ...
    if (now - lastTapRef.current < 300) {
      ...
      setDoubleTapAnimating(true); window.setTimeout(() => setDoubleTapAnimating(false), 750); return;
    }
    lastTapRef.current = now;
    window.setTimeout(() => { if (lastTapRef.current !== now) return; lastTapRef.current = 0; revealControls(); }, 300);
  }
  async function handleLike() { if (liking) return; setLiking(true); const nextLiked = !reel.liked; onLikeChange(reel.id, nextLiked, reel.likeCount + (nextLiked ? 1 : -1)); const result = await toggleReelLike(reel.id); setLiking(false); if (result.success) onLikeChange(reel.id, result.data.liked, reel.likeCount + (result.data.liked ? 1 : -1)); }
  ...
  return <div className="relative flex h-full w-full flex-shrink-0 items-center justify-center bg-black">
    ...
    {doubleTapAnimating && <div className="pointer-events-none absolute inset-0 z-30 flex items-center justify-center"><div className="absolute animate-ping opacity-30"><HeartIcon filled size={120} /></div><div className="animate-[heartPop_750ms_ease-out_forwards] drop-shadow-[0_8px_24px_rgba(0,0,0,0.35)]"><HeartIcon filled size={105} /></div></div>}
    ...
    <button onClick={toggleMute} className="absolute right-3 top-16 z-40 flex h-10 w-10 items-center justify-center rounded-full bg-black/60">{isMuted ? <MuteIcon /> : <UnmuteIcon />}</button>
    {showControls && <div className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center"><div className="pointer-events-auto flex items-center gap-7 rounded-2xl bg-black/75 px-6 py-5 shadow-2xl"><button aria-label="Back 10 seconds" onClick={(e) => seek(-10, e)}><SeekIcon forward={false} /></button><button aria-label="Play or pause" onClick={togglePlayback}><PlayIcon playing={!!videoRef.current && !videoRef.current.paused} /></button><button aria-label="Forward 10 seconds" onClick={(e) => seek(10, e)}><SeekIcon forward /></button></div></div>}
