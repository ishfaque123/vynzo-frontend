...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (isSwitching && window.google?.accounts?.id) {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-slate-50 px-4">
      <Script
        ...
        onLoad={() => { if (isSwitching) window.google?.accounts?.id?.disableAutoSelect(); }}
      />
      <div
        id="g_id_onload"
        data-client_id={process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID}
        data-callback="handleVynzoCredential"
        data-auto_select={isSwitching ? 'false' : 'true'}
      />
      ...
      {isSwitching && !loading && <p className="mb-4 text-sm text-slate-500">Choose a different Google account to continue.</p>}
