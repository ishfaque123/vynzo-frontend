...
export async function fetchReelById(reelId: string) {
  try {
    const res = await fetch(apiUrl(`/api/reels/${encodeURIComponent(reelId)}`), { credentials: 'include' });
    return await readResponse(res);
  } catch { return { success: false, error: { message: 'Unable to load reel.' } }; }
}
