...
function CommentRow({ comment, currentUserId, reelOwner, onReply, onChanged, depth = 0 }: { comment: CommentNode; currentUserId?: string; reelOwner: boolean; onReply: (comment: CommentNode) => void; onChanged: (topLevel: boolean) => void; depth?: number }) {
  ...
  async function saveEdit() { const text = editText.trim(); if (!text) return; const result = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/reels/comments/${encodeURIComponent(comment.id)}`, { method: 'PUT', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: text }) }).then((r) => r.json()).catch(() => ({ success: false })); if (result.success) { setEditing(false); onChanged(false); } }
