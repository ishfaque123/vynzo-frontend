...
export default function ReelCommentsModal({ reelId, reelOwner, currentUserId, comments, loading, hasMore, loadingMore, onLoadMore, onClose, onCountChange }: { reelId: string; reelOwner: boolean; currentUserId?: string; comments: CommentNode[]; loading: boolean; hasMore: boolean; loadingMore: boolean; onLoadMore: () => void; onClose: () => void; onCountChange: (delta: number) => void }) {
  ...
  return createPortal(
    <div className="z-50 flex items-end justify-center bg-black/50" style={wrapperStyle} onClick={onClose}>
      <div className="flex w-full max-w-xl min-h-0 flex-col rounded-t-2xl bg-white" style={{ height: modalHeight ? `${modalHeight}px` : '88vh', maxHeight: '100%', transform: `translateY(${dragY}px)`, transition: dragging || pullClosing ? 'none' : 'transform 0.2s ease' }} onClick={(e) => e.stopPropagation()}>
